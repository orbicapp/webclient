...
import {
  GENERATE_COURSE_FROM_FILE_MUTATION,
  GENERATE_COURSE_FROM_TEXT_MUTATION,
  GET_COURSE_GENERATION_STATUS_QUERY,
  PREVIEW_GENERATED_COURSE_QUERY,
} from "@/lib/graphql";
...
async function getCourseGenerationStatus(jobId: string) {
  const { data, errors } = await apolloClient.query<{
    getCourseGenerationStatus: CourseGenerationStatus;
  }>({
    query: GET_COURSE_GENERATION_STATUS_QUERY,
    variables: { jobId },
    fetchPolicy: "network-only",
  });
  return formatResult<CourseGenerationStatus>(
    data?.getCourseGenerationStatus,
    errors
  );
}
...
async function getPreviewGeneratedCourse(jobId: string) {
  const { data, errors } = await apolloClient.query<{
    previewGeneratedCourse: GeneratedCoursePreview;
  }>({
    query: PREVIEW_GENERATED_COURSE_QUERY,
    variables: { jobId },
    fetchPolicy: "network-only",
  });
  return formatResult<GeneratedCoursePreview>(
    data?.previewGeneratedCourse,
    errors
  );
}

import { FileUpload, StorageService } from "@/services/storage-service";
...
export class FileUploadClient {
  ...
  private async createFileRecord(file: File): Promise<FileResponse> {
    const [fileRecord, error] = await StorageService.createFile({
      filename: file.name,
      mimetype: file.type,
      size: file.size,
    });
  ...
  private async completeFileUpload(
    ...
    const [completedFile, error] = await StorageService.completeFileUpload({
      fileId,
      parts,
    });

    if (error || !completedFile) {
      throw new Error(error);
    }

...
const LevelNode: React.FC<{
  ...
  onClick?: () => void;
  previewMode?: boolean;
}> = ({ level, chapterNumber, levelNumber, size, onClick, previewMode = false }) => {
  ...
  return (
    ...
    >
      ...
      {isClickable && (
        <motion.div
          className={cn(
            "absolute inset-0 rounded-full ring-4 -inset-3",
            styles.ring
          )}
          animate={{
            scale: [1, 1.2, 1],
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 py-20 space-y-16">
        {chapterGroups.map((group, chapterIndex) => (
          <div key={group.chapter._id} className="space-y-12">
            ...
            <div 
              ...
              style={{
                gridTemplateColumns: `repeat(auto-fit, minmax(${nodeSize + 40}px, 1fr))`,
